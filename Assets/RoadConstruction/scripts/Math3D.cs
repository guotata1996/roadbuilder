...
public class Math3d
{
    ...
    public static Vector2 GetPointOnSpline(float percentage, Vector2[] cPoints)
    {
        ...
        else
        {
            ...
            {
                // 2nd order bezier
                return (1 - percentage) * (1 - percentage) * cPoints[0] + 2 * (percentage) * (1 - percentage) * cPoints[1] + percentage * percentage * cPoints[2];
            }
            else
            {
    ...
    // Modified from LineLine
    public static bool RayRayIntersection(out Vector3 intersection, Ray r1, Ray r2)
    {
        if (!LineLineIntersection(out intersection, r1.origin, r1.direction, r2.origin, r2.direction)){
            return false;
        }
        return Vector3.Dot(intersection - r1.origin, r1.direction) > -0.0001f && Vector3.Dot(intersection - r2.origin, r2.direction) > -0.0001f;
    }
